...
function validateFields(lastname_id, firstname1_id, pet_id){
    let result = true
    result = validateElement(lastname_id)
    result = validateElement(firstname1_id)
    result = validateElement(pet_id)
    return result
}
...
function validateElement(unverifiedElement) {
    const error = unverifiedElement.parent.querySelectorAll('.error')
